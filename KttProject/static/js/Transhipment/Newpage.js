...
function OutDeclarationChange() {
    ...
    $('#outwardTransportModeShowHide').show()
    $('#handlingAgentshow').hide()

    if (DeclarationVal == "BRE : BLANKET REMOVAL") {
    ...
    else if (DeclarationVal == "REM : REMOVAL") {
        $('#outwardTransportModeShowHide').hide()
        $('#OutCargoImporterShow').show()
    }
    else if (DeclarationVal == "TTF : THRU TRANSHIPMENT WITHIN SAME FTZ" || DeclarationVal == "TTI : THRU TRANSHIPMENT WITH INTER-GATEWAY MOVEMENT") {
        $('#handlingAgentshow').show()
        $('#OutWardCarrierAgentShow').show()
        $('#OutInwardCarrierShow').show()
    }
}
...
function ImportFocusOut() {
    ...
    $("#SummaryImporter").html("<p>" + $("#ImporterCruei").val() +"-" + $("#ImporterName").val() + "</p>");
}

function HandlingFocusIn() {
    var myValues = [];
    for (var i of Importer) {
        myValues.push(i.Code + ":" + i.Name);
    }
    Autocomplete1(myValues, "#handlingCode");
}

function HandlingFocusOut() {
    let Code = $("#handlingCode").val().trim()
    if (Code != "") {
        Importer.filter((obj) => {
            if (obj.Code == Code) {
                $("#handlingCruei").val(obj.CRUEI)
                $("#handlingName").val(obj.Name)
                $("#handlingName1").val(obj.Name1)
            }
        })
    }
    else {
    ...
    $("#SummaryHandling").html("<p>" + $("#handlingCruei").val() +"-" + $("#handlingName").val() + "</p>");
}
...
function Loadingporttrans(Head, Table, Input, Text) {
    $("#InNonImporterSerchId").show();
    var tag = "";
    for (var i of Table) {
      tag += `
...
function ItemOutDelHblHawb() {
    $("#Loading").show();
    $.ajax({
      url: "/TranDelHblHawb/" + $("#PermitId").val() + "/",
      success: function (response) {
        ItemReset();
...
function OutfinalSave() {
    ...
    try {
            const dataToSend = {
                ...
                HandlingAgentCode: $('#handlingCode').val().toUpperCase(),
                InwardCarrierAgentCode: $('#InwardCode').val().toUpperCase(),//
                OutwardCarrierAgentCode: $('#OutwardCode').val().toUpperCase(),
                FreightForwarderCode: $('#FrieghtCode').val().toUpperCase(),
                ClaimantPartyCode: "",//No field
                EndUserCode: $('#EndConsigneCode').val().toUpperCase(),
                ArrivalDate: ArrivalDate,

                // ArrivalTime: $('#ArrivalTime').val(),
                LoadingPortCode: $('#LoadingPortCode').val().toUpperCase(),
                VoyageNumber: $('#VoyageNumber').val().toUpperCase(),
                VesselName: $('#VesselName').val().toUpperCase(),
                OceanBillofLadingNo: $('#OceanBillofLadingNo').val(),
                ConveyanceRefNo: $('#ConveyanceRefNo').val().toUpperCase(),
                TransportId: $('#TransportId').val().toUpperCase(),
                FlightNO: $('#FlightNO').val().toUpperCase(),
                AircraftRegNo: $('#AircraftRegNo').val().toUpperCase(),
                ...
                FinalDestinationCountry: $('#FinalDestinationCountry').val(),//have to check
                OutVoyageNumber: $('#OutVoyageNumber').val().toUpperCase(),
                OutVesselName: $('#OutVesselName').val().toUpperCase(),
                OutOceanBillofLadingNo: $('#OutOceanBillofLadingNo').val().toUpperCase(),
                VesselType: $('#VesselType').val(),
                VesselNetRegTon: $('#VesselNetRegTon').val(),
                VesselNationality: $('#VesselNationality').val(),
                TowingVesselID: $('#TowingVesselID').val().toUpperCase(),
                TowingVesselName: $('#TowingVesselName').val(),
                NextPort: $('#NextPort').val().toUpperCase(),
                LastPort: $('#LastPort').val().toUpperCase(),
                ...
                GrossReference: $('#GrossReference').val().toUpperCase(),
                TradeRemarks: $('#summaryTradeRemarks').val(),
                InternalRemarks: $('#InternalRemarks').val(),
                DeclareIndicator: $('#DeclareIndicator').prop('checked'),
        ...
        // function summaryPreviousFunction() {
        //     var previousPermitNo = $("#PreviousPermitNo").val();
        //     var currentSummary = $("#summaryTradeRemarks").val();
        //     $("#summaryTradeRemarks").val(currentSummary + " PREVIOUS PERMIT NO: " + previousPermitNo);
        // }
...
function summaryEXRateFunction() {
    var unitPriceCurrencies = ItemData.map(item => item.UnitPriceCurrency);
    var exchangeRates = ItemData.map(item => item.ExchangeRate);
    var summaryTradeRemarksValue = ' Currency: ' +  unitPriceCurrencies.join(', ') + ', Exchange Rates: ' + exchangeRates.join(', '); 
    var currentSummary = $("#summaryTradeRemarks").val();
    $("#summaryTradeRemarks").val(currentSummary + " " + summaryTradeRemarksValue);
}
...
function cpcData() {
    ...
    const seaData = document.getElementsByName('OutSeaStoreName')
    if ($('#OutSeaStore').prop('checked')) {
        let RowNo = 1
        for (var i = 0; i < seaData.length; i = i + 3) {
            if (seaData[i].value != "") {
                cpcAlldata.push([RowNo, "SEA STORE", seaData[i].value, seaData[i + 1].value, seaData[i + 2].value])
                RowNo += 1
            }
        }
    }
...
function loadCpc() {
    if (CpcData.length > 0) {
        ...
        for (var i of CpcData) {
            ...
            if (i.CPCType == "SEA STORE") {
                $('#OutSeaStore').prop('checked', true)
                OutCpcHideShow('#OutSeaStore', '.OutSeaStoreClass', 'OutSeaStoreName')

                console.log("OutSeaStore values:");
                console.log("ProcessingCode1:", i.ProcessingCode1);
                console.log("ProcessingCode2:", i.ProcessingCode2);
                console.log("ProcessingCode3:", i.ProcessingCode3);
